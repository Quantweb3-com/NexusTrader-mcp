...
| `uv run nexustrader-mcp start` | 后台启动 MCP 服务器，等待上线确认 |
| `uv run nexustrader-mcp start --no-wait` | 后台启动，不等待上线 |
| `uv run nexustrader-mcp stop` | 停止后台服务器 |
| `uv run nexustrader-mcp status` | 查看服务器运行状态 |
| `uv run nexustrader-mcp logs [N]` | 查看最后 N 行日志（默认 50） |
| `uv run nexustrader-mcp serve` | 前台运行（调试用，Ctrl+C 停止） |
| `uv run nexustrader-mcp serve --config path/to/config.yaml` | 指定配置文件路径（前台） |
...
```powershell
$env:BINANCE_API_KEY="xxx"; $env:BINANCE_SECRET="yyy"; uv run nexustrader-mcp start
```
...
```powershell
# PowerShell
$env:PYTHONPATH=""; uv run nexustrader-mcp start
```
